...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .hero h1{font-size:clamp(38px,4.4vw,58px);line-height:1.04;letter-spacing:-.03em;margin:0 0 20px;font-weight:800;text-wrap:balance}
  .mission{font-size:clamp(16px,1.5vw,19px);line-height:1.55;color:#CFD8EA;max-width:520px;margin:0}
  ...
  .proof span{font-size:12.5px;color:#B9C5D8;line-height:1.35}
  .panel{padding:34px clamp(22px,4vw,60px);display:grid;place-items:center}
  .card{width:min(100%,460px);background:#fff;border:1px solid var(--line);border-radius:24px;padding:36px 34px;box-shadow:0 28px 70px -48px rgba(21,32,58,.45)}
  .card h2{font-size:29px;line-height:1.12;letter-spacing:-.025em;margin:0 0 9px}
  ...
  button.submit{width:100%;height:52px;border:none;border-radius:14px;background:var(--primary);color:#fff;font:700 15.5px 'Hanken Grotesk',system-ui;cursor:pointer;box-shadow:0 14px 28px -16px var(--primary);transition:.15s}
  ...
  @media (max-width:520px){.proof{grid-template-columns:1fr}.card{padding:28px 22px;border-radius:18px}}
</style>
</head>
<body>
<main class="shell">
  <section class="story">
    <div class="brand"><span class="mark"><span></span></span>EduDesk<b>&nbsp;AI</b></div>
    <div class="hero">
    ...
    <div class="proof">
      <div><b>24s</b><span>average resolution for everyday issues</span></div>
      <div><b>71%</b><span>resolved instantly, no waiting for IT</span></div>
      <div><b>3.2h</b><span>teacher time recovered each week</span></div>
    </div>
  </section>
  <section class="panel">
    <form class="card" method="post" action="/login">
      <h2>Sign in with your school email</h2>
      <p class="sub">Use an institution email to open your EduDesk AI dashboard. This demo checks the email format only.</p>
      __ERROR__
      ...
      <div class="hint">
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#12876A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3 5 6v5c0 4.4 3 7.6 7 9 4-1.4 7-4.6 7-9V6l-7-3z"/></svg>
        <span>For the demo, any realistic school, college, or university email opens the dashboard.</span>
      </div>
